// ==UserScript==
// @name         GRV Tools com Endereço e Policial 3 em 1 v1.4.0
...
/* =============================================================================
   ...
   ► CHANGELOG
     v1.4.0
       • Filtro por Situação aplicado NO SERVIDOR (API): sempre que o filtro do
         script for numérico, o request da LISTA recebe &status=<código>. Para
         “Retirados (qualquer)”, a query envia vários status= (100,101,102,104,
         105,107,108,109,110). Para “Todos”, não envia status.
       • Coleta por meta de mantidos: se Fim=5, o coletor pagina até juntar 5×100
         itens que passam no filtro do script (ou até a lista zerar), em vez de
         parar em 5 páginas cruas.
       • Mantido: força 100 por página (limit/perPage/pageSize/size = 100).
       • Padrão de intervalo na UI: Início=1, Fim=5 (você põe 0 quando quiser
         varrer até a primeira página vazia).

     v1.3.0
       • (obsoleto) meta de mantidos existia, mas ainda havia um limite oculto de
         “páginas cruas”. Removido nesta versão.
...
(function () {
  ...
  const STATE = {
    ...
    listBase: "",             // URL base da LISTA (sem page; MESMA origem para o DETALHE)
    baseLoggedOnce: false,    // log “✓ Base...” apenas 1x/ sessão
    rowsAOA: null,            // matriz para exportação
  };
  ...
  const FILTER_OPTIONS = [
    ["all",  "Todos"],
    ["1",    "No pátio"],     // padrão da UI
    ["0",    "Pendente"],
    ["2",    "Em leilão"],
    ["4",    "Clone"],
    ["5",    "Excluir"],
    ["6",    "Frustrado"],
    ["ret",  "Retirados (qualquer)"],
    ["100",  "Retirado pelo proprietário"],
    ["101",  "Retirado por leilão"],
    ["102",  "Retirado por decisão judicial"],
    ["104",  "Retirado pelo representante legal"],
  ...
  //    - filterValue "all": não adiciona nada.
  //    - filterValue "ret": adiciona status=100&status=101&... (um por item).
  //    - filterValue numérico: adiciona status=<num>.
  function applyServerFilterToUrl(u, filterValue){
    const url = new URL(u, location.origin);

    // Remove status pré-existentes
    ...
    if (filterValue === "ret") {
      for (const s of RET_STATUS) url.searchParams.append("status", String(s));
      return url.toString();
    }
    const want = Number(filterValue);
    if (Number.isFinite(want)) {
      url.searchParams.append("status", String(want));
    }
    return url.toString();
  }
  ...
  function pageUrl(base, n){
    ...
    // Impõe 100 por página em chaves comuns
  ...
  async function getListPage(base, n, filterValue){
    // 1) constrói URL da página com 100 por página
    let u = pageUrl(base, n);
    // 2) injeta filtro de Situação na query (server-side)
  ...
  // Mantemos o filtro client-side como segurança em caso da API ignorar status.
  ...
  function pickEnderecoFromList(item, origin){
    if (origin === "apreensao"){
      return [
        safe(item?.enderecoApreensao),
        safe(item?.bairroApreensao),
        safe(item?.cidadeApreensao),
        safe(item?.ufApreensao),
        safe(item?.complementoApreensao),
      ];
    } else { // remocao
      return [
        safe(item?.enderecoRemocao),
        safe(item?.bairroRemocao),
        safe(item?.cidadeRemocao),
        safe(item?.ufRemocao),
        safe(item?.complementoRemocao),
      ];
    }
  }
  ...
  function parsePolicialFromObservacao(text){
    ...
    let matObs = "-----";
    const matLabel = /(matr[ií]cula|mat\.?|mat|rg|id)\s*[:\-]?\s*([^\n]+)/i;
    const m = t.match(matLabel);
    if (m){
      const d = digits(m[2]);
      if (d.length>=5 && d.length<=9) matObs = d;
    } else {
      const m2 = t.match(/(\d[\d\.\-\/\s]{3,}\d)/);
      if (m2){
        const d = digits(m2[1]);
        if (d.length>=5 && d.length<=9) matObs = d;
      }
    ...
    let nomeObs = "-----";
    const nomeLabel = /(nome\s*(?:e\s*cargo|completo)?|policial|apreensor|agente|inspetor|oficial)\s*[:\-]?\s*([^\n]+)/i;
    const n = t.match(nomeLabel);
    if (n){
      let seg = n[2].split(/\b(?:matr[ií]cula|mat\.?|rg|id)\b/i)[0];
      seg = seg.replace(/\s{2,}/g," ").trim();
      if (seg) nomeObs = seg;
    } else {
      const line = t.split("\n").map(s=>s.trim()).find(s => s && !/^\d{5,}$/.test(digits(s)));
  ...
  function buildPanel(){
    ...
    panel.style.cssText="position:fixed;right:20px;bottom:20px;z-index:2147483647;background:#0d1117;color:#e6edf3;border:1px solid #30363d;border-radius:10px;width:610px;box-shadow:0 8px 20px rgba(0,0,0,.55);font-family:Segoe UI,Arial,sans-serif;font-size:13px;";
    panel.innerHTML=`
      <div style="padding:6px 10px;display:flex;gap:8px;align-items:center;border-bottom:1px solid #30363d;cursor:move;background:#010409;">
        <b style="flex:1;">GRV Tools com Endereço e Policial 3 em 1 v1.4.0</b>
      ...
      <textarea id="grv_log" readonly style="width:calc(100% - 16px);margin:0 8px 8px;height:240px;background:#0a0f14;color:#d1f7c4;border:1px solid #30363d;border-radius:6px;resize:vertical;font-family:ui-monospace,Consolas,Menlo,monospace;font-size:12px;padding:8px"></textarea>
    `;
    document.body.appendChild(panel);

    // arrastar painel
    const head=panel.firstElementChild; let sx=0,sy=0,ox=0,oy=0,drag=false;
    head.addEventListener("mousedown",e=>{drag=true;sx=e.clientX;sy=e.clientY;const r=panel.getBoundingClientRect();ox=r.left;oy=r.top;});
  ...
  async function onRun(){
    ...
    const tasks = []; // fila de detalhes → {rowIndex, item}

    while(true){
      if (targetKept != null && kept >= targetKept) break;   // já atingiu N×100 mantidos
      ...
      if (!items || items.length===0){
        ...
        empty++;
        // Fim=0: varre até “zerar” (primeira vazia).
        // Fim>0: acabou a lista antes da meta — paramos.
        break;
      }
      ...
      for (const it of items){
        // Defesa: mesmo filtrando na API, mantemos o filtro local
  ...
  // 2 workers fixos, com retries leves e backoff
    ...
    async function fetchWithRetry(job){
      ...
      for (let a=1; a<=MAX; a++){
        try { return await fetchDetalheByIdOrNumero(job.item); }
          ...
          if (a===MAX) throw e;
          const wait = 400 * a; // 0.4s, 0.8s, 1.2s
          await sleep(wait);
        }
      }
    }
  ...
  async function copyTSV(){
    ...
    try {
      if (navigator.clipboard && window.ClipboardItem) {
        ...
        log("Copiado (TSV). Cole no Google Sheets/Excel.");
        return;
      }
    } catch {}
    try {
