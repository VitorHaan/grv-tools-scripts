...
/* =============================================================================
   ...
   ► CHANGELOG (resumo relevante)
     v1.4.3 (este arquivo)
       • ⭐ NOVO: botão Minimizar/Restaurar no cabeçalho (também por duplo-clique).
       • ⭐ NOVO: Redimensionamento pelas bordas (E/D/Cima/Baixo) com limites.
       • Restante do fluxo inalterado (coleta, filtros, exportações, UI e layout).
...
(function () {
  ...
  const STATE = {
    ...
    listBase: "",             // URL base da LISTA
    baseLoggedOnce: false,    // log “✓ Base...” 1x/ sessão
    rowsAOA: null,            // matriz para exportação
    // ⭐ NOVO: estado visual do painel
    minimized: false,
    // guarda tamanho anterior ao minimizar para restaurar igual
    lastSize: { w: 610, h: 520 }, // valores padrão aproximados
  };
  ...
  // ⭐ NOVO: refs extras (minimizar/resize)
  let btnMin, bodyWrap;
  let resL, resR, resT, resB; // handles de borda
  ...
  function buildPanel(){
    ...
    panel.style.cssText=[
      ...
      "min-width:320px","min-height:100px" // ⭐ NOVO: limites mínimos
    ].join(";");
    ...
    panel.innerHTML=`
      <div id="grv_head" style="padding:6px 10px;display:flex;gap:8px;align-items:center;border-bottom:1px solid #30363d;cursor:move;background:#010409;">
        <b id="grv_title" style="flex:1;">GRV Tools com Endereço e Policial 3 em 1 v1.4.3</b>
      ...
      <!-- ⭐ NOVO: embrulho do corpo (facilita esconder/mostrar no minimizar) -->
      <div id="grv_body">
    ...
    // ⭐ NOVO: Handles de resize nas 4 bordas (largura 6px; altura 6px)
    for (const [id,css] of [
    ...
    // ⭐ NOVO: Minimizar/Restaurar
    btnMin.addEventListener("click", toggleMinimize);
    head.addEventListener("dblclick", toggleMinimize); // atalho pelo título

    // ⭐ NOVO: Resize pelas bordas
    attachResize(resL, "L");
    attachResize(resR, "R");
    attachResize(resT, "T");
    attachResize(resB, "B");
  ...
  // ⭐ NOVO: alterna estado minimizado sem afetar lógica de captura
  function toggleMinimize(){
    STATE.minimized = !STATE.minimized;
    if (STATE.minimized){
      // guarda tamanho atual para restaurar depois
      const r = panel.getBoundingClientRect();
      STATE.lastSize.w = r.width; STATE.lastSize.h = r.height;
      bodyWrap.style.display = "none";
      logEl && (logEl.style.display = "none"); // (já fica dentro do body)
      panel.style.height = "auto"; // encolhe ao cabeçalho
      btnMin.textContent = "+";
  ...
  // ⭐ NOVO: redimensionamento por bordas
  // Dicas de estudo:
  // - L/R ajustam largura; T/B ajustam altura.
  // - Em L/T também movemos a origem para preservar o lado oposto.
  // - Limites mínimos respeitados (min-width/height do painel).
  function attachResize(handle, side){
    ...
    const onUp = ()=>{ startRect=null; window.removeEventListener("mousemove", onMove); window.removeEventListener("mouseup", onUp); };
    handle.addEventListener("mousedown", (e)=>{
      // Evita conflito com drag do cabeçalho
